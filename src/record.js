...
class Record{
    ...
    constructor(db,table,primaryKey,id){
        ...
        this.id = id;
        this.db = new database('localhost','example','example',this.database);
    }
    _build(){
    ...
    _create(){
      return new Promise((resolve,reject)=>{
        ...
        let keys = Object.keys(insertion);
        keys.forEach((key)=>{
          if(typeof insertion[key] === 'string'){
            insertion[key] = insertion[key].replace(/'/g,"''");
          }
        });
    ...
    static _date(dateStr){
      const database = require('./db');
      return database.date(dateStr);
    }
}
